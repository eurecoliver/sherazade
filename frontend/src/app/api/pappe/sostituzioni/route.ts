...
export async function GET(request: NextRequest) {
  ...
  try {
    const { res } = await fetchBackend(request, `/api/v1/pappe/sostituzioni/${params ? `?${params}` : ''}`, { cache: 'no-store' } as RequestInit)
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, '/api/v1/pappe/sostituzioni/', {
      method: 'POST',
