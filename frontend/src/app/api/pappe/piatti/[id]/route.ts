...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { res } = await fetchBackend(request, `/api/v1/pappe/piatti/${id}/`, { cache: 'no-store' } as RequestInit)
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/pappe/piatti/${id}/`, {
      method: 'PATCH',
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/pappe/piatti/${id}/`, { method: 'DELETE' })
    if (res.status === 204) {
