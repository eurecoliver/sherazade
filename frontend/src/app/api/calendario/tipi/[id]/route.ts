...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/calendario/tipi/${params.id}/`, {
      method: 'PATCH',
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/calendario/tipi/${params.id}/`, { method: 'DELETE' })
