...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { res } = await fetchBackend(request, `/api/v1/calendario/${params.id}/`, { cache: 'no-store' } as RequestInit)
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/calendario/${params.id}/`, {
      method: 'PATCH',
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/calendario/${params.id}/`, { method: 'DELETE' })
