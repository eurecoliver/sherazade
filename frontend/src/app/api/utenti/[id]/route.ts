...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/auth/utenti/${params.id}/`, {
      method: 'PATCH',
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/auth/utenti/${params.id}/`, { method: 'DELETE' })
    if (res.status === 204) {
