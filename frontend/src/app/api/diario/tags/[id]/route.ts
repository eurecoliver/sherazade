...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/diario/tags/${id}/`, { method: 'DELETE' })
    if (res.status === 204) {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/diario/tags/${id}/`, {
      method: 'PATCH',
