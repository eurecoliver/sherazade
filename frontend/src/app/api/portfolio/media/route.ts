...
export async function GET(request: NextRequest) {
  ...
  try {
    const { res } = await fetchBackend(
      request,
      `/api/v1/portfolio/media/${params ? `?${params}` : ''}`,
...
export async function POST(request: NextRequest) {
  try {
    // Multipart — passa FormData direttamente senza Content-Type (fetch aggiunge boundary)
    const formData = await request.formData()
    const { res, newAccessToken } = await fetchBackend(request, '/api/v1/portfolio/media/', {
      method: 'POST',
      body: formData,
    })
