...
export async function GET(request: NextRequest) {
  try {
    const { res } = await fetchBackend(request, '/api/v1/portfolio/anni/', { cache: 'no-store' } as RequestInit)
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, '/api/v1/portfolio/anni/', {
      method: 'POST',
