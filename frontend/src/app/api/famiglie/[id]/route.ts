...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { res } = await fetchBackend(request, `/api/v1/famiglie/${id}/`, { cache: 'no-store' } as RequestInit)
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, `/api/v1/famiglie/${id}/`, {
      method: 'PATCH',
