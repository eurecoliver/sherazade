...
export async function GET(request: NextRequest) {
  try {
    const { res } = await fetchBackend(request, '/api/v1/presenze/qr-token-insegnanti/', { cache: 'no-store' } as RequestInit)
...
export async function POST(request: NextRequest) {
  try {
    const { res, newAccessToken } = await fetchBackend(request, '/api/v1/presenze/qr-token-insegnanti/', {
      method: 'POST',
