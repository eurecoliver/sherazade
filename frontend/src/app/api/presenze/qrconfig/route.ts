...
export async function GET(request: NextRequest) {
  try {
    const { res } = await fetchBackend(request, '/api/v1/presenze/qr-config/', { cache: 'no-store' } as RequestInit)
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const { res, newAccessToken } = await fetchBackend(request, '/api/v1/presenze/qr-config/', {
      method: 'PATCH',
