...
function TabQR({ isAdmin }: { isAdmin: boolean }) {
  ...
  // Carica config e token all'avvio
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    caricaToken()
    // Auto-rinnovo ogni 8 ore (token valido tutta la giornata, ma questo garantisce
    // che se il QR è esposto su un telefono il token sia sempre fresco)
  ...
  if (qrAbilitato === null) {
    return <div style={{ textAlign: 'center', padding: '2rem', color: '#0984E3' }}>Caricamento...</div>
  }
  ...
  return (
    <div>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: '0.8rem', color: '#718096', marginTop: '0.2rem' }}>
              {qrAbilitato ? 'Attivo — i genitori possono usare il QR' : 'Disabilitato — il QR non è visibile ai genitori'}
            </div>
          </div>
          <button
      ...
      {!qrAbilitato && (
        <div style={{ background: 'white', borderRadius: '16px', padding: '2rem', textAlign: 'center', color: '#718096' }}>
          <div style={{ fontSize: '2.5rem', marginBottom: '0.75rem' }}>🚫</div>
          <p style={{ margin: 0, fontWeight: 600 }}>QR Check-in disabilitato</p>
          {isAdmin && <p style={{ margin: '0.5rem 0 0', fontSize: '0.85rem' }}>Usa il toggle qui sopra per riattivarlo.</p>}
        </div>
      )}

      {qrAbilitato && (
        <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 2px 8px rgba(9,132,227,0.08)' }}>
          ...
          {loadingToken && (
            <div style={{ textAlign: 'center', padding: '3rem', color: '#0984E3' }}>Generazione QR...</div>
          )}
          ...
          {!loadingToken && checkinUrl && (
            <div id="qr-print-area" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
              ...
              <div style={{ fontSize: '0.75rem', color: '#718096', textAlign: 'center', wordBreak: 'break-all', maxWidth: '260px' }}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                <button
                  ...
                  style={{
                    padding: '0.625rem 1.25rem', background: '#0984E3', color: 'white',
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#EAF4FF' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '1rem 1rem 4rem' }}>
        ...
        {activeTab === 'qr' && (
          <TabQR isAdmin={['admin', 'direttrice'].includes(userRole)} />
        )}
      ...
      {!loading && righe.length > 0 && activeTab === 'presenze' && (
