...
function Overlay({ onClose, zIndex = 1000, children }: { onClose: () => void; zIndex?: number; children: React.ReactNode }) {
  return (
    ...
    >
      <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '20px', padding: '1.5rem', width: '100%', maxWidth: '600px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        {children}
      </div>
    </div>
  )
...
export default function GenitoriPage() {
  ...
  const carica = useCallback(async () => {
    ...
    try {
      ...
      // Famiglie: arricchisci con nomi bambini
  ...
  // Figli di un genitore
  const figlidiGenitore = (gId: number): Bambino[] => {
    const bambinoIds = famiglie
      .filter(f => f.genitore1 === gId || f.genitore2 === gId)
      .map(f => f.bambino)
    return bambini.filter(b => bambinoIds.includes(b.id))
  }
  ...
  const openEdit = (g: Genitore) => {
    setEditForm({ first_name: g.first_name, last_name: g.last_name, email: g.email, phone: g.phone ?? '', is_active: g.is_active })
    setEditError('')
    setShowEdit(true)
  }
    ...
    <div style={{ minHeight: '100vh', background: '#F8F9FA' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '1.25rem 1rem 3rem' }}>
        ...
        <div style={{ background: 'white', borderRadius: '14px', padding: '0.875rem 1rem', marginBottom: '1rem', display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          ...
          <button
            onClick={() => { setSelected(null); setShowEdit(false); setNewForm({ ...EMPTY_EDIT, password: '' }); setNewError(''); setShowNew(true) }}
            style={{ padding: '0.5rem 1.25rem', background: '#6C63FF', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 700, fontSize: '0.875rem', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}
          >
            + Nuovo genitore
          </button>
        </div>
            ...
            {filtered.map(g => {
              ...
              return (
                <div
                  ...
                  style={{ background: 'white', borderRadius: '16px', padding: '1.25rem', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '2px solid transparent', transition: 'all 0.15s', opacity: g.is_active ? 1 : 0.6 }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = '#6C63FF')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = 'transparent')}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', marginBottom: '0.75rem' }}>
                    <div style={{ width: 44, height: 44, borderRadius: '12px', background: '#EDE9FE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem', flexShrink: 0 }}>
                      👤
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ margin: 0, fontWeight: 700, color: '#333', fontSize: '0.95rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {nomeCompleto(g)}
                      </p>
                      <p style={{ margin: 0, fontSize: '0.78rem', color: '#888', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{g.email}</p>
                    </div>
                    {!g.is_active && (
                      <span style={{ background: '#FED7D7', color: '#C53030', fontSize: '0.7rem', fontWeight: 700, padding: '0.15rem 0.5rem', borderRadius: '6px' }}>INATTIVO</span>
                    )}
                  </div>
                  {g.phone && <p style={{ margin: '0 0 0.5rem', fontSize: '0.8rem', color: '#666' }}>📞 {g.phone}</p>}
                  {figli.length > 0 && (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#F7FAFC' }}>
                  {['Nome', 'Email', 'Telefono', 'Figli', 'Stato'].map(h => (
                ...
                {filtered.map(g => {
                  ...
                  return (
                    <tr
                      key={g.id}
                      onClick={() => setSelected(g)}
                      style={{ borderBottom: '1px solid #F7FAFC', cursor: 'pointer', opacity: g.is_active ? 1 : 0.6 }}
                    ...
                    >
                      ...
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.25rem' }}>
                          {figli.map(b => (
                            <span key={b.id} style={{ background: b.gruppo_colore + '20', color: b.gruppo_colore, padding: '0.15rem 0.4rem', borderRadius: '5px', fontSize: '0.75rem', fontWeight: 600 }}>
                              {b.nome} {b.cognome}
                            </span>
                          ))}
      ...
      {selected && !showEdit && (
        <Overlay onClose={() => setSelected(null)}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.25rem' }}>
            {[
              ['Nome', selected.first_name || '—'],
              ['Cognome', selected.last_name || '—'],
              ['Email', selected.email],
              ['Telefono', selected.phone || '—'],
              ['Stato', selected.is_active ? 'Attivo' : 'Inattivo'],
            ].map(([label, value]) => (
              <div key={label} style={{ background: '#F7FAFC', borderRadius: '10px', padding: '0.75rem 1rem' }}>
                <p style={{ margin: 0, fontSize: '0.72rem', fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</p>
                <p style={{ margin: '0.2rem 0 0', fontSize: '0.9rem', fontWeight: 600, color: '#333' }}>{value}</p>
              </div>
            ))}
          </div>
          ...
          {(() => {
            ...
            return figli.length > 0 ? (
              <div style={{ marginBottom: '1.25rem' }}>
                <p style={{ margin: '0 0 0.625rem', fontSize: '0.8rem', fontWeight: 700, color: '#555', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Figli</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                  {figli.map(b => (
                    <span
                      key={b.id}
                      style={{ background: b.gruppo_colore + '20', color: b.gruppo_colore, border: `1px solid ${b.gruppo_colore}50`, padding: '0.35rem 0.875rem', borderRadius: '10px', fontSize: '0.85rem', fontWeight: 700, cursor: 'pointer' }}
                      onClick={() => router.push(`/${locale}/dashboard/admin/bambini`)}
                      title="Vai all'anagrafica bambini"
                    >
              ...
              <p style={{ color: '#aaa', fontSize: '0.85rem', marginBottom: '1.25rem' }}>Nessun figlio associato.</p>
            )
          })()}

          {/* Azioni */}
          <div style={{ display: 'flex', gap: '0.625rem', flexWrap: 'wrap', borderTop: '1px solid #F7FAFC', paddingTop: '1rem' }}>
            ...
            {[
              ...
              <div key={key}>
                ...
                <input
                  ...
                  style={{ width: '100%', padding: '0.5rem 0.75rem', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '0.875rem', fontFamily: 'inherit', boxSizing: 'border-box' }}
                />
              </div>
            ))}
          </div>
      ...
      {showNew && (
        <Overlay onClose={() => setShowNew(false)}>
          ...
          <p style={{ margin: '0 0 1rem', fontSize: '0.85rem', color: '#888' }}>Crea un account genitore. Se omessa, la password verrà impostata come non utilizzabile (il genitore potrà fare il reset tramite email).</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            {[
